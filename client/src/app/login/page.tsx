...
const Page = () => {
  ...
  return (
    <main className="flex flex-col flex-nowrap select-none font-sans">
      ...
      {isLoading && <Loader />}
        <div>
          <h2 className="text-5xl font-extrabold mb-2">Login</h2>
          <h4 className="text-gray-600 mb-4">Hi , Welcome Back 👋 </h4>
          <div className="max-w-[350px] flex flex-col gap-4">
            <LoginForm toastMsg={toastMsg} setIsLoading={setIsLoading}/>
            ...
            <Button className="bg-transparent text-gray-600 border-[1px] border-gray-400 rounded-xl w-full gap-4 items-center justify-center ">
              <Image src={google} width={20} height={20} alt="" />
              Login With Google
            </Button>
            <p className="text-gray-400 text-sm mx-auto">
              Dont have an account?{"  "}
              <Link href={`${process.env.APP_URL}/auth/signup`}>
                <span className="font-bold text-secondary">Sign Up</span>
              </Link>
            </p>
          </div>
        </div>
